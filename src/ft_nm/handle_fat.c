...
/*   Updated: 2018/01/23 16:15:49 by lfabbro          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		has_64(void *ptr, struct fat_header *header,
		struct fat_arch *arch_ptr, int options)
{
	uint32_t	i;
	...
	while (i < header->nfat_arch)
	{
		...
		{
			nm_handle_64((void *)ptr + arch_ptr->offset, options);
			return (1);
		}
...
int				nm_handle_fat(void *ptr, int options)
{
	...
	if (has_64(ptr, header, arch_ptr, options) == 1)
		return (EXIT_SUCCESS);
	else
	{
		while (i < header->nfat_arch)
		{
			if (arch_ptr->cputype == CPU_TYPE_I386)
				nm_handle_32((void *)ptr + arch_ptr->offset, options);
			else
